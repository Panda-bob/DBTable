...
bool ServerInfo::parse_json(XJsonValue& json)
{
	if(json.type() != XJsonValue::objectValue) return false;
...
bool ServerInfo::to_json(XJsonValue& json)
{
	json.type() = XJsonValue::objectValue;
...
bool SecurityServerInfo::parse_json(XJsonValue& json)
{
	if(json.type() != XJsonValue::objectValue) return false;
	...
	// m_key
	XJsonValue& key = json.as_objects()["key"];
	if (key.type() == XJsonValue::nullValue) return true;
	if (key.type() != XJsonValue::stringValue) return false;
	m_key = (string)key.as_string();

	return true;
}

bool SecurityServerInfo::to_json(XJsonValue& json)
{
	json.type() = XJsonValue::objectValue;
	...
	// m_key
	json.as_objects()["key"] = XJsonValue(m_key);
...
bool ListenInfo::parse_json(XJsonValue& json)
{
	if(json.type() != XJsonValue::objectValue) return false;

	// m_ip
	XJsonValue& ip = json.as_objects()["ip"];
	if (ip.type() == XJsonValue::nullValue) return true;
	if (ip.type() != XJsonValue::stringValue) return false;
	m_ip = (string)ip.as_string();
...
bool ListenInfo::to_json(XJsonValue& json)
{
	json.type() = XJsonValue::objectValue;

	// m_ip
	json.as_objects()["ip"] = XJsonValue(m_ip);
...
bool SecurityListenInfo::parse_json(XJsonValue& json)
{
	if(json.type() != XJsonValue::objectValue) return false;
...
bool SecurityListenInfo::to_json(XJsonValue& json)
{
	json.type() = XJsonValue::objectValue;
...
int AliyunSmsInfo::parse_bytes(const byte* src, uint32 len)
{
	if (src == NULL) return -1;
	if (len < sizeof(uint32)) return -1;
	uint32 msglen = *(const uint32*)src;
	...
	// m_secret
	if (decoder.remain() == 0) return msglen;
	if (!decoder.get_string(m_secret)) return -1;

	// m_sign_name
	if (decoder.remain() == 0) return msglen;
	if (!decoder.get_string(m_sign_name)) return -1;

	// m_template_code
	if (decoder.remain() == 0) return msglen;
	if (!decoder.get_string(m_template_code)) return -1;

	return msglen;
}

int AliyunSmsInfo::to_bytes(byte* dst, uint32 maxlen)
{
	if (dst == NULL) return -1;
	if (maxlen < sizeof(uint32)) return -1;
	Aone2Coder coder(dst + sizeof(uint32), maxlen - sizeof(uint32));
	...
	// m_secret
	if (!coder.set_string(m_secret)) return -1;

	// m_sign_name
	if (!coder.set_string(m_sign_name)) return -1;

	// m_template_code
	if (!coder.set_string(m_template_code)) return -1;

	*(uint32*)dst = (coder.position() - dst);
	return *(uint32*)dst;
}

uint32 AliyunSmsInfo::calc_length()
{
	uint32 length = 0;
	...
	// m_secret
	length += Aone2Coder::size_string(m_secret);

	// m_sign_name
	length += Aone2Coder::size_string(m_sign_name);

	// m_template_code
	length += Aone2Coder::size_string(m_template_code);

	return length + sizeof(uint32);
}

bool AliyunSmsInfo::parse_json(XJsonValue& json)
{
	if(json.type() != XJsonValue::objectValue) return false;

	// m_id
	XJsonValue& id = json.as_objects()["id"];
	if (id.type() == XJsonValue::nullValue) return true;
	if (id.type() != XJsonValue::stringValue) return false;
	m_id = (string)id.as_string();

	// m_secret
	XJsonValue& secret = json.as_objects()["secret"];
	if (secret.type() == XJsonValue::nullValue) return true;
	if (secret.type() != XJsonValue::stringValue) return false;
	m_secret = (string)secret.as_string();

	// m_sign_name
	XJsonValue& sign_name = json.as_objects()["sign_name"];
	if (sign_name.type() == XJsonValue::nullValue) return true;
	if (sign_name.type() != XJsonValue::stringValue) return false;
	m_sign_name = (string)sign_name.as_string();

	// m_template_code
	XJsonValue& template_code = json.as_objects()["template_code"];
	if (template_code.type() == XJsonValue::nullValue) return true;
	if (template_code.type() != XJsonValue::stringValue) return false;
	m_template_code = (string)template_code.as_string();

	return true;
}

bool AliyunSmsInfo::to_json(XJsonValue& json)
{
	json.type() = XJsonValue::objectValue;

	// m_id
	json.as_objects()["id"] = XJsonValue(m_id);

	// m_secret
	json.as_objects()["secret"] = XJsonValue(m_secret);

	// m_sign_name
	json.as_objects()["sign_name"] = XJsonValue(m_sign_name);

	// m_template_code
	json.as_objects()["template_code"] = XJsonValue(m_template_code);
...
///////////////////////////////////////////////////////////////////////////////
Config_GameServer::Config_GameServer()
{
}
...
int Config_GameServer::parse_bytes(const byte* src, uint32 len)
{
	if (src == NULL) return -1;
	if (len < sizeof(uint32)) return -1;
	uint32 msglen = *(const uint32*)src;
...
int Config_GameServer::to_bytes(byte* dst, uint32 maxlen)
{
	if (dst == NULL) return -1;
	if (maxlen < sizeof(uint32)) return -1;
	Aone2Coder coder(dst + sizeof(uint32), maxlen - sizeof(uint32));
...
uint32 Config_GameServer::calc_length()
{
	uint32 length = 0;
...
bool Config_GameServer::parse_json(XJsonValue& json)
{
	if(json.type() != XJsonValue::objectValue) return false;

	// m_listen_aone_rc4
	XJsonValue& listen_aone_rc4 = json.as_objects()["listen_aone_rc4"];
	if (listen_aone_rc4.type() == XJsonValue::nullValue) return true;
	if (!m_listen_aone_rc4.parse_json(listen_aone_rc4)) return false;

	// m_listen_http
	XJsonValue& listen_http = json.as_objects()["listen_http"];
	if (listen_http.type() == XJsonValue::nullValue) return true;
	if (!m_listen_http.parse_json(listen_http)) return false;
	...
	// m_sms_info
	XJsonValue& sms_info = json.as_objects()["sms_info"];
	if (sms_info.type() == XJsonValue::nullValue) return true;
	if (!m_sms_info.parse_json(sms_info)) return false;

	// m_jiguang_push_appkey
	XJsonValue& jiguang_push_appkey = json.as_objects()["jiguang_push_appkey"];
	if (jiguang_push_appkey.type() == XJsonValue::nullValue) return true;
	if (jiguang_push_appkey.type() != XJsonValue::stringValue) return false;
	m_jiguang_push_appkey = (string)jiguang_push_appkey.as_string();

	// m_jiguang_push_secret
	XJsonValue& jiguang_push_secret = json.as_objects()["jiguang_push_secret"];
	if (jiguang_push_secret.type() == XJsonValue::nullValue) return true;
	if (jiguang_push_secret.type() != XJsonValue::stringValue) return false;
	m_jiguang_push_secret = (string)jiguang_push_secret.as_string();

	// m_npc_server
	XJsonValue& npc_server = json.as_objects()["npc_server"];
	if (npc_server.type() == XJsonValue::nullValue) return true;
	if (!m_npc_server.parse_json(npc_server)) return false;

	return true;
}

bool Config_GameServer::to_json(XJsonValue& json)
{
	json.type() = XJsonValue::objectValue;

	// m_listen_aone_rc4
	XJsonValue& listen_aone_rc4 = json.as_objects()["listen_aone_rc4"];
	if (!m_listen_aone_rc4.to_json(listen_aone_rc4)) return false;

	// m_listen_http
	XJsonValue& listen_http = json.as_objects()["listen_http"];
	if (!m_listen_http.to_json(listen_http)) return false;
	...
	// m_sms_info
	XJsonValue& sms_info = json.as_objects()["sms_info"];
	if (!m_sms_info.to_json(sms_info)) return false;

	// m_jiguang_push_appkey
	json.as_objects()["jiguang_push_appkey"] = XJsonValue(m_jiguang_push_appkey);

	// m_jiguang_push_secret
	json.as_objects()["jiguang_push_secret"] = XJsonValue(m_jiguang_push_secret);

	// m_npc_server
	XJsonValue& npc_server = json.as_objects()["npc_server"];
	if (!m_npc_server.to_json(npc_server)) return false;
...
///////////////////////////////////////////////////////////////////////////////
Config_NPCServer::Config_NPCServer()
{
	// empty
}
...
int Config_NPCServer::parse_bytes(const byte* src, uint32 len)
{
	if (src == NULL) return -1;
	if (len < sizeof(uint32)) return -1;
	uint32 msglen = *(const uint32*)src;
	...
	// m_game_server
	if (decoder.remain() == 0) return msglen;
	int game_server_ret = m_game_server.parse_bytes(decoder.position(), decoder.remain());
	if (game_server_ret <= 0) return game_server_ret;
	if (!decoder.set_pos(decoder.position() + game_server_ret)) return -1;

	return msglen;
}

int Config_NPCServer::to_bytes(byte* dst, uint32 maxlen)
{
	if (dst == NULL) return -1;
	if (maxlen < sizeof(uint32)) return -1;
	Aone2Coder coder(dst + sizeof(uint32), maxlen - sizeof(uint32));
	...
	// m_game_server
	int game_server_ret = m_game_server.to_bytes(coder.position(), coder.remain());
	if (game_server_ret <= 0) return game_server_ret;
	coder.set_pos(coder.position() + game_server_ret);

	*(uint32*)dst = (coder.position() - dst);
	return *(uint32*)dst;
}

uint32 Config_NPCServer::calc_length()
{
	uint32 length = 0;
	...
	// m_game_server
	length += m_game_server.calc_length();

	return length + sizeof(uint32);
}

bool Config_NPCServer::parse_json(XJsonValue& json)
{
	if(json.type() != XJsonValue::objectValue) return false;
	...
	// m_game_server
	XJsonValue& game_server = json.as_objects()["game_server"];
	if (game_server.type() == XJsonValue::nullValue) return true;
	if (!m_game_server.parse_json(game_server)) return false;

	return true;
}

bool Config_NPCServer::to_json(XJsonValue& json)
{
	json.type() = XJsonValue::objectValue;
	...
	// m_game_server
	XJsonValue& game_server = json.as_objects()["game_server"];
	if (!m_game_server.to_json(game_server)) return false;
