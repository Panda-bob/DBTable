...
#define ASSERT(x) \
	do { \
		if (!(x)) { \
			XERROR("Assertion ("#x") failed at %s(..) in %s:%d, LastError:%u\r\n", __FUNCTION__, __FILE__, __LINE__, ::GetLastError()); \
			abort();\
		} \
	} while (0)
...
#define VERIFY(x) \
	do { \
		if (!(x)) { \
			XERROR("Verification ("#x") failed at %s(..) in %s:%d, LastError:%u\r\n", __FUNCTION__, __FILE__, __LINE__, ::GetLastError()); \
		} \
	} while (0)
...
#define ASSERT(x) \
	do { \
		if (!(x)) { \
			XERROR("Assertion ("#x") failed at %s(..) in %s:%d, errno:%d\r\n", __FUNCTION__, __FILE__, __LINE__, errno); \
		} \
	} while (0)
...
#define VERIFY(x) \
	do { \
		if (!(x)) { \
			XERROR("Verification ("#x") failed at %s(..) in %s:%d, errno:%d\r\n", __FUNCTION__, __FILE__, __LINE__, errno); \
		} \
	} while (0)
#endif//__WINDOWS__
#else//__DEBUG__
